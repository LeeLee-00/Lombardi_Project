# This is a helper file to fdp.py and lombardi.py
...
# parsefile check
# Attempted r'E:\LombardiSpringEmbedder\includingDummyVertexAlgorithm\input\runGraph.dot'


# support dot or graphml 
def parseFile(file):
    initParse()  # originally 'r'
    f = open(r'E:\LombardiSpringEmbedder\includingDummyVertexAlgorithm\input\runGraph.dot', 'r').read() # update to your filepath to hit the runGraph.dot file
    regex: str = r"(\w+) -- (\w+)" if file[-3:] == "dot" else r"source=\"(\w+)\" target=\"(\w+)\""
    # if decide to init nodes, must do first cuz edge auto add
    for x, y in re.findall(regex, f):
        Edge(x, y)
...
def bezier(pp, steps=30):
    """
    Calculate a Bézier curve from 4 control points and return a list of the resulting points. 2007 Victor Blomqvist
